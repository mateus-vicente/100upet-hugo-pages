...
- `detectors_file`:
  Location of the file describing the detector configuration (introduced in [Section 3.3](.././03_detector_configuration)).
...
- `log_level`:
  ...
  [Section 3.8](.././08_logging_and_verbosity). Can be overwritten by the `-v` parameter on the command line (see
  [Section 3.5](.././05_allpix_executable)).

- `log_format`:
  Determines the log message format to display. Possible options are `SHORT`, `DEFAULT` and `LONG`, where all options are
  case-insensitive. More information can be found in [Section 3.8](.././08_logging_and_verbosity).
...
- `log_file`:
  ...
  on the command line using the `-l` parameter (see [Section 3.5](.././05_allpix_executable)).
...
- `library_directories`:
  ...
  [Section 4.4](../../04_framework/04_modules#module-instantiation) for more information.
...
- `multithreading`:
  ...
  [Section 4.3](../../04_framework/04_modules#multithreading-parallel-execution-of-events).
...
- `buffer_per_worker`:
  ...
  the correct order can be guaranteed (see [Section 4.10](../../04_framework/10_multithreading)). Defaults to `256`.
